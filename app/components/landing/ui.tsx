...
export function SectionShell({
  ...
  // El borde de seam SOLO va en el lado que NO está flush con la sección vecina (Problema+
  // Agitación deben seguir leyéndose como un solo bloque, sin costura entre ellas — 55 T1).
  const bordeTop = elevacion === 'elevada' && flush !== 'top' ? 'border-t' : '';
  const bordeBottom = elevacion === 'elevada' && flush !== 'bottom' ? 'border-b' : '';
  return (
    <section
      id={id}
      aria-label={ariaLabel}
      className={`${elevacion === 'elevada' ? `bg-[var(--surface)] ${bordeTop} ${bordeBottom} border-[color-mix(in_oklab,var(--text-tertiary)_14%,transparent)]` : ''} ${pt} ${pb} ${className}`}
    >
...
export function CtaButton({
  ...
  // Feedback de navegación (defecto real, reportado varias veces por el revisor-visual):
  // el <a> hace una carga de página completa; sin este estado, whileTap se revierte al
  // soltar el dedo y la persona no sabe si el tap se registró mientras carga la siguiente página.
      ...
      {visible && (
        ...
        >
          <motion.a
            ...
            onClick={() => {
              if (ofertaVista) setCargando(true);
            }}
            aria-busy={cargando}
          ...
          >
            {cargando && <Loader2 size={16} strokeWidth={2.4} className="animate-spin" aria-hidden="true" />}
