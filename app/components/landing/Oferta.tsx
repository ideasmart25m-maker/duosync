...
export function Oferta({
  ...
  // El CTA mensual es un link con estilo propio (outline, menos peso que el anual — 55), así
  // que no usa <CtaButton> pero SÍ necesita el mismo feedback de navegación (defecto real: se
  // quedaba mudo tras el whileTap mientras cargaba /onboarding).
        ...
        <div className="mx-auto mt-10 grid max-w-[880px] grid-cols-1 items-start gap-6 md:grid-cols-2">
          ...
          >
            ...
            <motion.a
              ...
              onClick={() => setCargandoMensual(true)}
              aria-busy={cargandoMensual}
              className={`mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-[var(--radius-button)] border border-[color-mix(in_oklab,var(--accent)_45%,transparent)] text-[16px] font-semibold text-[var(--accent)] transition-colors duration-150 hover:bg-[var(--chip-bg)] [touch-action:manipulation] ${
                cargandoMensual ? 'opacity-70' : ''
              }`}
            >
              {cargandoMensual && <Loader2 size={16} strokeWidth={2.4} className="animate-spin" aria-hidden="true" />}
              {mensual.ctaLabel}
            </motion.a>
          </motion.div>
        </div>
