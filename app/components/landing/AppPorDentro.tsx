...
// DESVIACIÓN pedida por el usuario (2026-08-14, anotada en ESTADO.md): autoplay
// que avanza solo cada ~3.2s, en loop, pausado mientras el usuario toca/pasa el
// mouse por el carrusel y desactivado por completo con prefers-reduced-motion.
...
export function AppPorDentro({
  ...
  // Mueve SOLO el scroll horizontal del carrusel (nunca scrollIntoView: eso
  // puede arrastrar el scroll VERTICAL de toda la página si el navegador
  // decide que el contenedor "nearest" es el body — bug real detectado al
  // probar el autoplay, que secuestraba el scroll de la página cada 3.2s).
  const irA = (i: number): void => {
    const scroller = scrollerRef.current;
    const frame = frameRefs.current[i];
    if (!scroller || !frame) return;
    const destino = frame.offsetLeft - (scroller.clientWidth - frame.clientWidth) / 2;
    scroller.scrollTo({ left: destino, behavior: reduce ? 'auto' : 'smooth' });
  };

  // Autoplay: avanza solo en loop cada 3.2s; se pausa con hover/touch/foco del
  // usuario y no arranca si prefers-reduced-motion está activo.
  const [pausado, setPausado] = useState(false);
  const activoRef = useRef(activo);
  activoRef.current = activo;
  useEffect(() => {
    if (reduce || pausado || frames.length <= 1) return;
    const t = setInterval(() => {
      irA((activoRef.current + 1) % frames.length);
    }, 3200);
    return () => clearInterval(t);
  }, [reduce, pausado, frames.length]);
  ...
  return (
    <SectionShell id={id} elevacion="elevada" ariaLabel="La app por dentro">
      <motion.div variants={contenedor} initial="hidden" whileInView="visible" viewport={VIEWPORT_ONCE}>
        ...
        {/* La pausa del autoplay envuelve frames Y dots: un usuario de teclado
            que tabula hasta los dots también debe pausar antes de interactuar
            (WCAG 2.2.2) — no basta con hover/focus solo dentro del scroller. */}
        <motion.div
          variants={item}
          className="mt-10"
